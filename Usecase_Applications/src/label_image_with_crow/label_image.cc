...
#include "tensorflow/lite/examples/label_image_with_crow/label_image.h"
...
#include "tensorflow/lite/examples/label_image_with_crow/bitmap_helpers.h"
#include "tensorflow/lite/examples/label_image_with_crow/get_top_n.h"
#include "tensorflow/lite/examples/label_image_with_crow/log.h"
...
std::string SetupInterpreter(Settings* settings, const DelegateProviders& delegate_providers) {
  // Execution time
  std::string execution_time_results;
  ...
  if(settings->execution_duration) {
    ...
    execution_time_results += "TIME: Setup Interpreter: Load Model: "
              + std::to_string(get_us(stop_time) - get_us(start_time))
              + " us\n";
    gettimeofday(&start_time, nullptr);
  }
  ...
  if(settings->execution_duration) {
    gettimeofday(&stop_time, nullptr);
    execution_time_results += "TIME: Setup Interpreter: Configure and Build Interpreter: "
              + std::to_string(get_us(stop_time) - get_us(start_time))
              + " us\n";
    gettimeofday(&start_time, nullptr);
  }
  ...
  if(settings->execution_duration) {
    gettimeofday(&stop_time, nullptr);
    execution_time_results += "TIME: Setup Interpreter: Build profiler and delegation: "
              + std::to_string(get_us(stop_time) - get_us(start_time))
              + " us\n";
    gettimeofday(&g_stop_time, nullptr);
    execution_time_results += "TIME: Setup Interpreter: Global time: "
              + std::to_string(get_us(g_stop_time) - get_us(g_start_time))
              + " us\n";
  }
...
std::string RunInference(Settings* settings, const DelegateProviders& delegate_providers, const std::string& input_image, ResultsAndExecutionTime *res_and_exec_time) {
  // Execution time
  std::string execution_time_results;
  struct timeval start_time, stop_time, g_start_time, g_stop_time;
  if(settings->execution_duration) {
  ...
  if(settings->execution_duration) {
    gettimeofday(&stop_time, nullptr);
    execution_time_results += "TIME: Run Inference: Read BMP: "
              + std::to_string(get_us(stop_time) - get_us(start_time))
              + " us\n";
    gettimeofday(&start_time, nullptr);
  }
  ...
  if(settings->execution_duration) {
    gettimeofday(&stop_time, nullptr);
    execution_time_results += "TIME: Run Inference: Resize image and copy it to input tensor: "
              + std::to_string(get_us(stop_time) - get_us(start_time))
              + " us\n";
    gettimeofday(&start_time, nullptr);
  }
  ...
  if(settings->execution_duration) {
    gettimeofday(&stop_time, nullptr);
    execution_time_results += "TIME: Run Inference: Warm-up interpreter: "
              + std::to_string(get_us(stop_time) - get_us(start_time))
              + " us\n";
    gettimeofday(&start_time, nullptr);
  }
  ...
  if(settings->execution_duration) {
    gettimeofday(&stop_time, nullptr);
    execution_time_results += "TIME: Run Inference: Total invokation time: "
              + std::to_string(get_us(stop_time) - get_us(start_time))
              + " us\n";
    execution_time_results += "TIME: Run Inference: Average invokation time per loop: "
              + std::to_string((get_us(stop_time) - get_us(start_time)) /
                    (settings->loop_count))
              + " us\n";
    gettimeofday(&start_time, nullptr);
  ...
  if(settings->execution_duration) {
    gettimeofday(&stop_time, nullptr);
    execution_time_results += "TIME: Run Inference: Profiling: "
              + std::to_string(get_us(stop_time) - get_us(start_time))
              + " us\n";
    gettimeofday(&start_time, nullptr);
  }
  ...
  if(settings->execution_duration) {
    gettimeofday(&stop_time, nullptr);
    execution_time_results += "TIME: Run Inference: Rerieve results from output tensors: "
              + std::to_string(get_us(stop_time) - get_us(start_time))
              + " us\n";
    gettimeofday(&start_time, nullptr);
  }
  ...
  if(settings->execution_duration) {
    gettimeofday(&stop_time, nullptr);
    execution_time_results += "TIME: Run Inference: Read labels from file: "
              + std::to_string(get_us(stop_time) - get_us(start_time))
              + " us\n";
    gettimeofday(&start_time, nullptr);
  }
  ...
  if(settings->execution_duration) {
    gettimeofday(&stop_time, nullptr);
    execution_time_results += "TIME: Run Inference: Print results: "
              + std::to_string(get_us(stop_time) - get_us(start_time))
              + " us\n";
    gettimeofday(&g_stop_time, nullptr);
    execution_time_results += "TIME: Run Inference: Global time: "
              + std::to_string(get_us(g_stop_time) - get_us(g_start_time))
              + " us\n";
  }
 ...
 */
crow::response handle_image(const crow::request& req, CrowSettings *crow_settings, std::string& output_image, ResultsAndExecutionTime *res_and_exec_time) {
  std::string execution_time_results;
  ...
  if (crow_settings->execution_duration) {
    gettimeofday(&stop_time, nullptr);
    execution_time_results += "TIME: Crow: Handling and saving input image: "
              + std::to_string(get_us(stop_time) - get_us(start_time))
              + " us\n";
  }
...
crow::response process_image(CrowSettings *crow_settings, Settings *tflite_settings, DelegateProviders *delegate_providers, std::string& image, ResultsAndExecutionTime *res_and_exec_time) {
  std::string execution_time_results;
  struct timeval start_time, stop_time;
  if(crow_settings->execution_duration)
    gettimeofday(&start_time, nullptr);
  
  std::string results = RunInference(tflite_settings, *delegate_providers, image, res_and_exec_time);

  if (crow_settings->execution_duration) {
    gettimeofday(&stop_time, nullptr);
    execution_time_results += "TIME: Crow: Run inference: "
              + std::to_string(get_us(stop_time) - get_us(start_time))
              + " us\n";
  }
...
crow::response label_image(CrowSettings *crow_settings, Settings *tflite_settings, DelegateProviders *delegate_providers, const crow::request& req, ResultsAndExecutionTime *res_and_exec_time) {
  std::string saved_image;
  std::string results_accumulateur;
  crow::response res = handle_image(req, crow_settings, saved_image, res_and_exec_time);
  if (res.code != 200) { return res; }
  res = process_image(crow_settings, tflite_settings, delegate_providers, saved_image, res_and_exec_time);
  if (res.code != 200) { return res; }
  

  if (crow_settings->execution_duration) {
    return crow::response(crow::status::OK, res_and_exec_time->execution_time + res_and_exec_time->results);
  } else {
    return crow::response(crow::status::OK, res_and_exec_time->results);
  }
}
...
int Main(int argc, char **argv) {
    ...
    // Execution time
    // ResultsAndExecutionTime res_and_exec_time = {"", setup_interpreter_exec_time};

    CROW_ROUTE(app, "/")([](){
        return "Hello world";
    });

    CROW_ROUTE(app, "/label_image").methods(crow::HTTPMethod::POST)([&crow_settings, &tflite_settings, &delegate_providers, &setup_interpreter_exec_time](const crow::request& req){
        ResultsAndExecutionTime res_and_exec_time = {"", setup_interpreter_exec_time};
        return label_image(&crow_settings, &tflite_settings, &delegate_providers, req, &res_and_exec_time);
    });
