...
namespace object_detection {
    ObjectDetector::ObjectDetector(Settings s) {
        ...
        if (outputs.size() != 1) {
            std::cerr << "ERROR: Detection model graph should have 1 outputs, got " << std::to_string(outputs.size()) << std::endl;
            exit(EXIT_FAILURE);
        }

        // Match output tensors
        output = interpreter->tensor(outputs[0]);
        ...
        std::cout << "INFO: Output dims size: " << std::to_string(output->dims->size) << std::endl;
        for (int i = 0; i < output->dims->size; i++) {
            std::cout << "INFO: Output data[" << std::to_string(i) << "]: " << std::to_string(output->dims->data[i]) << std::endl;
        }
        std::cout << "INFO: Output data: " << std::to
        std::cout << "INFO: Quantization Params: Scale=" << std::to_string(output->params.scale) << " Zero_point=" << std::to_string(output->params.zero_point) << std::endl;
    ...
    void ObjectDetector::detect(Mat image, std::vector<DetectionResult> *results, float threshold_score) {
        ...
        // const int num_detections = (int) *f->data.f;
        // results->clear();
