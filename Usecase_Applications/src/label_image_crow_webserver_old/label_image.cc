...
class DelegateProviders {
 public:
  ...
  void MergeSettingsIntoParams(const TfliteSettings& s) {
...
void SetupInterpreter(TfliteSettings* settings, const DelegateProviders& delegate_providers) {
  // Execution time
...
std::string RunInference(TfliteSettings* settings, const DelegateProviders& delegate_providers, const std::string input_image) {
  // Execution time
  struct timeval start_time, stop_time, g_start_time, g_stop_time;
  if(settings->execution_duration) {
    gettimeofday(&g_start_time, nullptr);
...
crow::response handle_image(const crow::request& req, CrowSettings *crow_settings) {
...
crow::response process_image(std::string in_file, CrowSettings *crow_settings, TfliteSettings *tflite_settings, DelegateProviders *delegate_providers) {
    if (crow_settings->execution_duration)
        auto start = std::chrono::high_resolution_clock::now();
    
    std::string results = RunInference(tflite_settings, *delegate_providers);
...
crow::response label_image(CrowSettings *crow_settings, TfliteSettings *tflite_settings, crow::request& req) {
    crow::response res = handle_image(req, );
...
void parse_configfile_to_settings(const std::string& filename, CrowSettings *crow_settings, TfliteSettings *tflite_settings) {
    ...
    if (config.contains("tflite")) {
        ...
        if (tflite_config.contains("accelerated")) tflite_settings->input_file = tflite_config["accelerated"];
        if (tflite_config.contains("allow_fp16")) tflite_settings->input_file = tflite_config["allow_fp16"];
        if (tflite_config.contains("count")) tflite_settings->input_file = tflite_config["count"];
        if (tflite_config.contains("verbose")) tflite_settings->input_file = tflite_config["verbose"];
        if (tflite_config.contains("image")) tflite_settings->input_file = tflite_config["image"];
        if (tflite_config.contains("labels")) tflite_settings->input_file = tflite_config["labels"];
        if (tflite_config.contains("tflite_model")) tflite_settings->input_file = tflite_config["tflite_model"];
        if (tflite_config.contains("threads")) tflite_settings->input_file = tflite_config["threads"];
        if (tflite_config.contains("input_mean")) tflite_settings->input_file = tflite_config["input_mean"];
        if (tflite_config.contains("input_std")) tflite_settings->input_file = tflite_config["input_std"];
        if (tflite_config.contains("max_profiling_buffer_entries")) tflite_settings->input_file = tflite_config["max_profiling_buffer_entries"];
        if (tflite_config.contains("warmup_runs")) tflite_settings->input_file = tflite_config["warmup_runs"];
        if (tflite_config.contains("gl_backend")) tflite_settings->input_file = tflite_config["gl_backend"];
        if (tflite_config.contains("hexagon_delegate")) tflite_settings->input_file = tflite_config["hexagon_delegate"];
        if (tflite_config.contains("xnnpack_delegate")) tflite_settings->input_file = tflite_config["xnnpack_delegate"];
        if (tflite_config.contains("execution_duration")) tflite_settings->input_file = tflite_config["execution_duration"];
    }
}
...
int Main(int argc, char **argv) {
    ...
    TfliteSettings tflite_settings;
    ...
    CROW_ROUTE(app, "/label_image").methods(crow::HTTPMethod::POST)([&crow_settings, &tflite_settings](const crow::request& req){
