...
usage() {
    ...
    echo "      --duration <int>:                       The duration of each iteration for the experiment, in seconds. (default: 10)"
...
perform_measurement() {
    ...
    for iter in $(seq 1 $ITERATIONS); do
        ...
        (for i in ${!SENDER_SSH[@]}; do
            ...
            $SSH_COMMAND $sender "echo $'\n# ITERATION $iter\n' >> $REMOTE_WORKDIR/sender_${FILE_BASENAME}_${sender}_to_${receiver}_${NUMBER_DEVICES}${FILE_EXTENSION}; iperf3 -c $receiver_ip --time $DURATION --omit $OMIT --logfile $REMOTE_WORKDIR/sender_${FILE_BASENAME}_${sender}_to_${receiver}_${NUMBER_DEVICES}${FILE_EXTENSION}" &
        done
        wait
        )
        sleep 5
