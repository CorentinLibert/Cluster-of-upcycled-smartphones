...
def build_graph(df: pd.DataFrame, metdium_type: str, filename: str):
    ...
    sns.lineplot(data=df, x='icmp_seq', y='latency_ms', hue='receiver_id', err_style="bars", palette='Dark2')
    plt.title(f"Latency between a sender and several devices connected over {metdium_type}\nfor an increasing number of connected devices.")
    plt.xlabel('Number of devices connected to the network')
    plt.ylabel('Latency (in ms)')
    plt.legend(title='Receiver ID')
